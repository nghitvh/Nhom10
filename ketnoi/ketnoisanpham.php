...
function get_sanpham($sp_id)
{
    ...
    // Câu truy vấn lấy tất cả sinh viên
    $sql = "select * from sanpham where masp = {$sp_id}";
...
// Hàm thêm sinh viên
function add_sanpham($masp, $tensp, $giaban)
{
    // Gọi tới biến toàn cục $conn
    global $conn;
...
// Hàm sửa sinh viên
function edit_sp($masp, $tensp, $giaban)
{
    // Gọi tới biến toàn cục $conn
    global $conn;
    ...
    $giaban = addslashes($giaban);
    // Câu truy sửa
    $sql = "UPDATE sanpham SET tensp = '$tensp', giaban = '$giaban' WHERE masp = '$masp'";
